...
<div class="mb-20">
    <x-split-screen>
        <x-split-screen-left>  
            <div class="group shadow border-b-8">
                <button class="flex w-full items-center justify-between gap-2 bg-primary px-4 py-2 font-semibold tracking-wide text-white first-letter:text-xl">
                ...
                <div class="max-h-0 px-4 group-focus-within:max-h-52 transition-all  overflow-auto">
                <a href="{{route('explore.users')}}" class="py-2 inline-block mt-4 hover:underline text-primary">View and search for more users.</a>
                @forelse ($users as $user)
                <div class="my-4 rounded bg-gray-100 px-4 py-2 text-sm text-gray-600">
                    <div class="mb-1">
                    <a class="font-semibold hover:underline" href="/profile/{{ $user->username }}">{{$user->name}} <span class="font-normal">{{'@'.$user->username}}</span></a>
                    </div>
                    <div class="space-x-2">
            ...
            <div class="group shadow border-b-8">
                ...
                <div class="max-h-0 px-4 group-focus-within:max-h-80 transition-all  overflow-auto">
                    <a href="{{route('explore.posts')}}" class="py-2 inline-block mt-4 hover:underline text-primary">View and search for more posts.</a>
                    <div class="space-y-6">
            ...
            <div class="group shadow border-b-8">
                ...
                <div class="max-h-0 px-4 group-focus-within:max-h-52 transition-all  overflow-auto">
                    <a href="{{route('explore.communities')}}" class="py-2 inline-block mt-4 hover:underline text-primary">View and search for more communities.</a>
                    @forelse ($communities as $community)
                    <div class="my-4 rounded bg-gray-100 px-4 py-2 text-sm text-gray-600">
                        <div class="my-1">
                        <a class="font-semibold hover:underline" href="{{route('community.show',['handle_name' => $community->handle_name])}}">{{$community->name}}<span class="font-normal">{{'@'.$community->handle_name}}</span></a>
                        </div>
                        <div class="space-x-2">

        ...
        <div class="flex flex-wrap items-start gap-4">
          <div class="relative w-32 md:w-44">
            @if ($profile->profileImage())
              <img class="block h-32 w-full rounded-full object-cover md:h-44" src="{{ asset($profile->profileImage()) }}"
                   alt="profile image" />
            @else
              <div
                   class="flex h-32 w-full items-center justify-center rounded-full bg-primary uppercase text-white md:h-44">
          ...
          <div class="text-sm">
            <div>
              <div class="flex items-center gap-2 text-xl font-semibold">{{ $profile->user->name }}
                @if ($follows)
                  <span class="rounded-lg bg-gray-200 px-2 py-1.5 text-xs capitalize text-gray-500">follows you</span>
                @endif
              </div>
    ...
    <x-split-screen-right>
      <div class="py-4">
        <div class="relative mx-auto w-16">
          @if ($profile->profileImage())
            <img class="block h-16 w-full rounded-full object-cover" src="{{ asset($profile->profileImage()) }}"
                 alt="profile image" />
          @else
